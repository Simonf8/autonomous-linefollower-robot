...
class TrackerSensor:
    ...
    def __init__(self, left_pin=4, center_pin=2, right_pin=1, threshold=3000):
        ...
        # Working threshold from your testing (12-bit range)
        ...
        print(f"Threshold: {threshold} - Line detected when below this value (12-bit range: 0-4095)")
    
    def read_raw(self):
        """Read raw ADC values from the 3 physical sensors (12-bit)."""
        left_raw = self.left_sensor.read()
        center_raw = self.center_sensor.read()
        right_raw = self.right_sensor.read()
    ...
    def is_line_detected(self):
        ...
        left = 1 if self.left_sensor.read() < self.threshold else 0
        center = 1 if self.center_sensor.read() < self.threshold else 0
        right = 1 if self.right_sensor.read() < self.threshold else 0
    ...
    def print_sensor_debug(self):
        ...
        left = 1 if left_raw < self.threshold else 0
        center = 1 if center_raw < self.threshold else 0
        right = 1 if right_raw < self.threshold else 0
        
        print(f"[{left} {center} {right}] Raw:[{left_raw:4d} {center_raw:4d} {right_raw:4d}]")
...
class OmniwheelRobot:
    def __init__(self):
        ...
        self.tracker = TrackerSensor(left_pin=4, center_pin=2, right_pin=1, threshold=3000)
