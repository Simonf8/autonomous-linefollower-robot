...
ROI_ZONES = [
    {"height_ratio": 0.3, "top_offset": 0.65, "weight": 2.0},
    {"height_ratio": 0.25, "top_offset": 0.45, "weight": 1.0},
]
...
SPEEDS = {'FAST':'s', 'NORMAL':'n', 'SLOW':'S', 'TURN':'T', 'STOP':'H'}

STEERING_DEADZONE = 0.01
OFFSET_THRESHOLDS = {'PERFECT':0.04, 'GOOD':0.12, 'MODERATE':0.25, 'LARGE':0.45}
...
current_speed_cmd, current_turn_cmd = SPEEDS['STOP'], "F"
...
class BlackLinePIDController:
    ...
    def calculate(self, error, dt=None):
        ...
        smoothed_error = np.median(list(self.error_history)) if len(self.error_history) >= 3 else error
        self.integral = np.clip(self.integral + smoothed_error * dt, -self.integral_max, self.integral_max)
    ...
    def reset(self):
        self.prev_error, self.integral = 0.0, 0.0
        self.error_history.clear(); self.last_time = time.time()
...
class ESP32Communicator:
    def __init__(self, ip, port):
        ...
        self.sock = None
        self.last_speed_cmd, self.last_turn_cmd = None, None
        self.connect_attempts, self.max_connect_attempts = 0, 3
        self.connect()
    def connect(self):
        if self.sock:
            try: self.sock.close()
        ...
        try:
            ...
            self.connect_attempts = 0; self.last_speed_cmd=None; self.last_turn_cmd=None
            return True
        except Exception as e:
            logger.error(f"❌ ESP32 conn failed (Att {self.connect_attempts+1}): {e}")
            self.sock = None; self.connect_attempts += 1
            return False
    def send_command(self, speed_cmd, turn_cmd):
        if not self.sock:
            if self.connect_attempts >= self.max_connect_attempts:
                if int(time.time()) % 5 == 0: 
                    logger.info("Attempting ESP32 reconnect (periodic)...")
                    if not self.connect(): return False
                else: return False
            elif not self.connect(): return False
        try:
            ...
            if self.last_turn_cmd != turn_cmd:
                ...
                sent_something = True
            return True
        except socket.timeout: logger.error("💥 ESP Send Timeout"); self.sock=None; self.connect_attempts=0; return False
        except socket.error as e: logger.error(f"💥 ESP Send SocketErr: {e}"); self.sock=None; self.connect_attempts=0; return False
        except Exception as e: logger.error(f"💥 ESP Send GenErr: {e}"); self.sock=None; self.connect_attempts=0; return False
    def close(self):
        if self.sock:
            try:
                stop_cmd=SPEEDS['STOP']+"F"; logger.info(f"Sending STOP ({stop_cmd}) to ESP32.")
                self.sock.sendall(stop_cmd.encode()); time.sleep(0.05)
            except Exception as e: logger.error(f"⚠️ ESP StopErr on close: {e}")
            finally:
                try: self.sock.close()
                except: pass
                self.sock=None; logger.info("🔌 ESP32 socket closed.")
        else: logger.info("🔌 ESP32 not connected or already closed.")
...
def preprocess_for_black_lines(frame):
    ...
    # Reverted to user's original CLAHE values (from their initial script)
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
    enhanced = clahe.apply(gray)
    blurred = cv2.GaussianBlur(enhanced, (5,5), 0)
    bright_mask_val, bright_mask = cv2.threshold(blurred, 200, 255, cv2.THRESH_BINARY)
    if cv2.countNonZero(bright_mask) > 0: blurred = cv2.inpaint(blurred, bright_mask, 3, cv2.INPAINT_TELEA)
    
    _, binary_simple = cv2.threshold(blurred, BLACK_THRESHOLD, 255, cv2.THRESH_BINARY_INV)
    binary_adaptive = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, ADAPTIVE_BLOCK_SIZE, ADAPTIVE_C)
    combined = cv2.bitwise_or(binary_simple, binary_adaptive)
    
    # Reverted to user's original kernel_close size
...
def extract_roi_zones(frame):
    ...
    for z in ROI_ZONES:
        ...
        rt = max(0,min(rt,h-1)); rh = max(1,min(rh,h-rt))
        rois.append({'roi':frame[rt:rt+rh,:], 'top':rt, 'height':rh, 'weight':z["weight"]})
    return rois
...
def detect_black_lines(roi_data):
    ...
    for roi_info in roi_data:
        ...
        edges = cv2.bitwise_or(cv2.Canny(roi,CANNY_LOW,CANNY_HIGH), cv2.Canny(roi,CANNY_LOW//2,CANNY_HIGH//2))
        lines = cv2.HoughLinesP(edges,1,np.pi/180,max(HOUGH_THRESHOLD//2,10),minLineLength=max(HOUGH_MIN_LINE_LENGTH//2,15),maxLineGap=HOUGH_MAX_LINE_GAP*2)
        if lines is None: continue
        valid_lines=[]
        for line_seg in lines:
            x1,y1,x2,y2 = line_seg[0]; length = np.hypot(x2-x1,y2-y1)
            if length<10: continue
            angle = np.degrees(np.arctan2(y2-y1,x2-x1)); angle = angle+180 if angle<0 else angle
            # Angle filter: Reverted to 45 degrees deviation from vertical
            if abs(angle-90) > 45: 
                continue
            cx = (x1+x2)/2; pos_sc=1.0-(abs(cx-roi_w/2)/(roi_w/2)); len_sc=min(length/(roi_h*0.75),1.0)
            tot_sc=(pos_sc*0.7+len_sc*0.3)*length
            valid_lines.append({'coords':(x1,y1,x2,y2),'angle':angle,'center_x':cx,'score':tot_sc,'roi_top_offset':roi_top_abs})
        if valid_lines:
            valid_lines.sort(key=lambda x:x['score'],reverse=True)
            best_zone_lns = valid_lines[:min(3,len(valid_lines))]
            if best_zone_lns:
                avg_cx = np.mean([l['center_x'] for l in best_zone_lns])
                sx=sum(l['score']*math.cos(math.radians(l['angle'])) for l in best_zone_lns) 
                sy=sum(l['score']*math.sin(math.radians(l['angle'])) for l in best_zone_lns)
                avg_ang_rad = math.atan2(sy,sx)
                current_avg_angle = math.degrees(avg_ang_rad); current_avg_angle = current_avg_angle+180 if current_avg_angle<0 else current_avg_angle
                norm_off=(avg_cx-roi_w/2)/(roi_w/2); conf_roi=(min(len(best_zone_lns)/2.0,1.0))*weight
                if conf_roi > highest_weighted_conf:
                    highest_weighted_conf=conf_roi; best_offset,best_angle=norm_off,current_avg_angle; all_lines_info=best_zone_lns
    
    # Simplified confidence score based on number of lines in the best ROI
    num_lines_from_best_roi = len(all_lines_info)
    if num_lines_from_best_roi >= 3:
        final_conf = 0.95
    elif num_lines_from_best_roi == 2:
        final_conf = 0.65
    elif num_lines_from_best_roi == 1:
        final_conf = 0.35 # This will be > 0.15, so "Follow Line"
    else:
        final_conf = 0.0
    return best_offset, best_angle, all_lines_info, final_conf

# -----------------------------------------------------------------------------
# --- MOVEMENT CONTROL HELPER ---
# -----------------------------------------------------------------------------
def get_turn_command(steering_output):
    if abs(steering_output)<STEERING_DEADZONE: return 'F'
    elif steering_output<-0.05: return 'L'
    elif steering_output>0.05: return 'R'
    else: return 'F'
...
def draw_car_arrow(display_frame):
    ...
    if current_turn_cmd == 'L': arrow_color=(0,255,255); arrow_text=f"LEFT ({current_steering:.2f})"; arrow_angle_deg = -25+(current_steering*30)
    elif current_turn_cmd == 'R': arrow_color=(255,255,0); arrow_text=f"RIGHT ({current_steering:.2f})"; arrow_angle_deg = 25+(current_steering*30)
...
def draw_status_panel(display_frame):
    ...
    smap={v:k for k,v in SPEEDS.items()}; sdisp=smap.get(current_speed_cmd,"?").title(); tdisp="Fwd" if current_turn_cmd=="F" else("Left" if current_turn_cmd=="L" else "Right")
...
def main():
    ...
    try:
        while True:
            ...
            final_offset, final_angle, detected_line_segments, confidence = detect_black_lines(roi_zones_data)
            ...
            if final_offset is not None and confidence > 0.15: 
                ...
                if abs(current_steering) < STEERING_DEADZONE: current_turn_cmd='F'
                elif current_steering < -0.05: current_turn_cmd='L'
                elif current_steering > 0.05: current_turn_cmd='R'
                else: current_turn_cmd='F'
                ...
                search_counter+=1
                if search_counter < 7: current_turn_cmd='L' if last_known_offset<0 else 'R'; current_speed_cmd=SPEEDS['NORMAL']
                elif search_counter < 14: current_turn_cmd='R' if last_known_offset<0 else 'L'; current_speed_cmd=SPEEDS['NORMAL']
                elif search_counter < 28: current_turn_cmd='L' if (search_counter//4)%2==0 else 'R'; current_speed_cmd=SPEEDS['SLOW']
                else:
                    current_turn_cmd='F'; current_speed_cmd=SPEEDS['SLOW']
